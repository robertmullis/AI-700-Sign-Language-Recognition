...
fixed_kernels = np.repeat(fixed_kernels, 1, axis=2)       # shape (3,3,1,4)

# CNN
model = Sequential([
    # Fixed filters layer (not trainable)
    Conv2D(4, (3,3), padding='same', use_bias=False, 
           trainable=False, input_shape=(IMG_SIZE[0], IMG_SIZE[1], 1)),
...
#model.save("slr_cnn_model.keras")   # saves architecture + weights + optimizer state
#print("Model saved.")
...
present_classes = sorted(set(y_true_val) | set(y_pred_val))
present_labels = [letters[i] for i in present_classes]
cm_val = confusion_matrix(y_true_val, y_pred_val, labels=present_classes)

plt.figure(figsize=(10, 8))
disp = ConfusionMatrixDisplay(confusion_matrix=cm_val, display_labels=present_labels)
...
for i in range(num_images):
    plt.subplot(4, 3, i+1)
    plt.imshow(X_subset[i].reshape(IMG_SIZE[0], IMG_SIZE[1]), cmap='gray')
...
def overlay_gradcam(img_float, heatmap):
    h, w = img_float.shape[:2]
    # Convert grayscale to RGB for blending
    img_uint8 = (img_float.squeeze() * 255).astype(np.uint8)
...
def preprocess_img(img):
    ...
    return resized.reshape(1, IMG_SIZE[0], IMG_SIZE[1], 1) / 255.0
